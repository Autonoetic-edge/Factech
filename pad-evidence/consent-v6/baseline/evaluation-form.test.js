...
async function setup(value=ready, origin='https://facetech.31.97.186.120.sslip.io') {
  ...
  elements.evaluationHome.href='https://facetech.31.97.186.120.sslip.io/?v=ui-v5';
  ...
  const clock = makeClock(), calls=[], window={};
  ...
  vm.runInNewContext(code,{window,location,URL,Event,crypto,AbortController,document:{getElementById:id=>elements[id]},fetch,setTimeout:clock.setTimeout,clearTimeout:clock.clear});
  await clock.drain();
  return {window,e:elements,clock,calls,state,location};
}
...
test('only consented recording is offered and checkbox remains unchecked initially',async()=>{
  ...
  await assert.rejects(s.window.facetechCaptureOptions(),/Select.*Save this attempt/);
...
test('selected consent supplies metadata and a fresh credentialed no-store status request',async()=>{
  ...
  assert.match(s.e.consentState.textContent,/Consent selected/);
...
test('subject is validated before recording',async()=>{
  const s=await setup();s.e.captureConsent.checked=true;s.e.testSubject.value='';
  await assert.rejects(s.window.facetechCaptureOptions(),/person code/);
});
...
test('pending checks are coalesced and withdrawn consent cannot proceed',async()=>{
  ...
  const rejected=assert.rejects(a,/Select.*Save this attempt/);
