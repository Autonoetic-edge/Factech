...
test('actual form plus page: checked consent reaches enrollment metadata and receipt then resets',async()=>{
  ...
  assert.equal(p.el('captureConsent').checked,false);
  await p.click('verify');assert.match(p.text('result'),/Select.*Save this attempt/);
  assert.equal(p.posts().length,1);assertIdle(p);
});
