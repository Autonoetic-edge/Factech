#!/bin/sh
set -eu
release=/opt/facetech-releases/eval-pad-943dc39ab702
previous=/opt/facetech-releases/eval-pad-80a3dfa1dfe9
test ! -e "$release"
mkdir -p "$release"
tar -xzf /tmp/facetech-pad-slow-release.tar.gz -C "$release"
...
stage.write_text(patch_env(env.read_text(),{'FACETECH_CAPTURE_DIR':'/var/lib/facetech/pad-stage-943dc39ab702','CADDY_HTTP_PORT':'8100'}))
stage.chmod(0o600)
PY
install -d -m 700 -o 10002 -g 10002 /var/lib/facetech/pad-stage-943dc39ab702
cd "$release"
