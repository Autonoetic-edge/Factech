...
def check_liveness(
    ...
    signals = {
        s.name: s
        for s in (
            _duplicate_signal(frames, images, crops),
            _motion_signal(crops),
            _landmark_signal(detections),
            _timing_signal(frames),
...
def _duplicate_signal(frames: list[dict], images: list, crops: list) -> Signal:
    ...
    return Signal(
        name="duplicates",
        score=_ramp(max(sizes.values(), default=0), MAX_IDENTICAL_FRAMES - 0.5, 1.0),
        detail={
            "frames": len(frames),
...
def _motion_signal(crops: list) -> Signal:
    diffs = [
...
def _timing_signal(frames: list[dict]) -> Signal:
    ...
    return Signal(
        ...
        detail={
            ...
            "expected_cadence_ms": CAPTURE_CADENCE_MS,
        },
    )
