...
def test_ordered_actual_landmark_rotation(sign):
    frames = [{"ts_ms": i * 700} for i in range(12)]
    params = {"settle_ms": 1400, "switch_ms": 4200, "first_sign": sign, "target": 0.2}
    detections = [
        detection(0 if i < 3 else 3 * sign if i < 6 else -3 * sign) for i in range(12)
    ]
