...
export function createFaceSession(options: FaceSessionOptions): FaceSession {
  ...
  async function attempt(
    ...
    try {
      ...
      if (remaining() < (sequence ? 7700 : CAPTURE_SPAN_MS) + LANDING_MS + EXPIRY_MARGIN_MS) {
      ...
      frames = await captureFrames({
        env, video,
        intervalMs: sequence ? 700 : undefined,
        onFrame: (index, total) => emit({ type: 'frame', index, total }),
        onTick: (elapsed) => {
          const probe = readProbe();
          if (sequence) {
            const settle = challenge.params?.['settle_ms'] ?? 1400;
            const change = challenge.params?.['switch_ms'] ?? 4200;
            const sign = challenge.params?.['first_sign'] ?? 1;
            const direction = (elapsed < change ? sign : -sign) > 0 ? 'LEFT' : 'RIGHT';
            const cue = elapsed <= settle ? 'Face forward. Hold still.' : `Turn your head slightly to your ${direction} and hold. Keep the phone still.`;
