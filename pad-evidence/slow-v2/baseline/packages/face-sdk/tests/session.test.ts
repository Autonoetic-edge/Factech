...
test('head sequence binds identity/operation, emits ordered cues and records 700ms cadence', async () => {
  for (const sign of [-1, 1]) {
    const h = session();
    h.fetch.reply('/v1/challenge', { body: { ...CHALLENGE, action: 'HEAD_SEQUENCE',
      params: { settle_ms: 1400, switch_ms: 4300, first_sign: sign, target: .2 } } });
    ...
    assert.equal(scan.frames.at(-1)!.ts_ms - scan.frames[0]!.ts_ms, 7700);
  }
});
