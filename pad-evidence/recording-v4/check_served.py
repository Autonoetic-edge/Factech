...
assert 'ui-v4' in page and 'captureNoSave' not in page and 'Process without saving' not in page
assert 'id="retryRecording"' in page and 'id="consentState"' in page
for file in ['evaluation-form.js','page.js']:
    served=get('/'+file+'?v=ui-v4')
    local=open('/app/apps/integration-demo/'+file,'rb').read()
    assert served==local
status=json.loads(get('/capture-status?check=recording-v4'))
...
print(json.dumps({'ui':'ui-v4','consent_only':True,'retry_and_diagnostics':True,'served_files_match':True,'status':{k:status[k] for k in ['enabled','evaluation_only','consent_version','retention_days','total','write_failures','bank_failures','purge_failures']}}))
