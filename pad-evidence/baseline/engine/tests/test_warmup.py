...
def test_warm_models_is_a_noop_when_files_are_absent(monkeypatch):
    monkeypatch.setattr(detect, "_detector", None)
    monkeypatch.setattr(embed, "_embedder", None)
    monkeypatch.setattr("app.main.DET_MODEL_FILE", THIS_FILE.parent / "nope.onnx")
    monkeypatch.setattr("app.main.EMB_MODEL_FILE", THIS_FILE.parent / "nope.onnx")

    assert warm_models() == {"detector": False, "embedder": False}
    assert not detect.is_loaded() and not embed.is_loaded()


def test_app_starts_without_models(monkeypatch):
    monkeypatch.setattr("app.main.DET_MODEL_FILE", THIS_FILE.parent / "nope.onnx")
    monkeypatch.setattr("app.main.EMB_MODEL_FILE", THIS_FILE.parent / "nope.onnx")
...
def test_warm_models_loads_both_when_present():
    assert warm_models() == {"detector": True, "embedder": True}
