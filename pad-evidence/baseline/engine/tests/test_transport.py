...
@requires_models
def test_msgpack_enroll_then_verify_round_trip(low_det_thresh):
    scan = fixture_scan("face_like.jpg")
    r = post_msgpack("/v1/enroll", scan, user_id="alice")
    assert r.status_code == 200
    assert 0.0 <= r.json()["quality"]["score"] <= 1.0

    v = post_msgpack("/v1/verify", scan, user_id="alice")
    assert v.status_code == 200
    assert v.json()["match"] is True
    assert v.json()["score"] <= 1.0


@requires_models
def test_enroll_msgpack_verify_json_cross_transport(low_det_thresh):
    scan = fixture_scan("face_like.jpg")
    assert post_msgpack("/v1/enroll", scan, user_id="bob").status_code == 200
    v = post_json("/v1/verify", scan, user_id="bob")
    assert v.status_code == 200
    assert v.json()["match"] is True


@requires_models
def test_enroll_json_verify_msgpack_cross_transport(low_det_thresh):
    scan = fixture_scan("face_like.jpg")
    assert post_json("/v1/enroll", scan, user_id="carol").status_code == 200
    v = post_msgpack("/v1/verify", scan, user_id="carol")
    assert v.status_code == 200
    assert v.json()["match"] is True


@requires_models
def test_transports_produce_identical_scores(low_det_thresh):
    scan = fixture_scan("face_like.jpg")
    post_json("/v1/enroll", scan, user_id="dave")
    j = post_json("/v1/verify", scan, user_id="dave").json()
    m = post_msgpack("/v1/verify", scan, user_id="dave").json()
    assert j["score"] == pytest.approx(m["score"])
    assert j["match"] == m["match"]
    assert j["threshold"] == m["threshold"]


@requires_models
def test_msgpack_no_face_422(low_det_thresh):
    r = post_msgpack("/v1/enroll", fixture_scan("blank.jpg"))
    assert r.status_code == 422
...
@requires_models
def test_legacy_packed_enroll_verify_round_trip(low_det_thresh):
    scan = fixture_scan("face_like.jpg")
    assert post_legacy_msgpack("/v1/enroll", scan, user_id="erin").status_code == 200
    v = post_legacy_msgpack("/v1/verify", scan, user_id="erin")
    assert v.status_code == 200
    assert v.json()["match"] is True


@requires_models
def test_legacy_and_modern_packing_score_identically(low_det_thresh):
    scan = fixture_scan("face_like.jpg")
    post_json("/v1/enroll", scan, user_id="frank")
    modern = post_msgpack("/v1/verify", scan, user_id="frank").json()
    legacy = post_legacy_msgpack("/v1/verify", scan, user_id="frank").json()
    assert legacy["score"] == pytest.approx(modern["score"])
