...
@requires_models
def test_f02_undetectable_settle_window_is_low_quality_not_enrolled(low_det_thresh):
    scan = _scan_with_params(
        "face_like.jpg", {"settle_ms": 2400, "target": 0.25}, blank_first=5
    )
    r = client.post(
        "/v1/enroll", json={"user_id": "mallory", "facescan": scan_to_b64(scan)}
    )
    assert r.status_code == 422, r.json()
    assert r.json()["error"]["code"] == "LOW_QUALITY"
...
def test_f03_verify_does_not_match_the_bystander(fake_pipeline):
    store.enroll("bob", _VEC["bob"])
    r = client.post(
        "/v1/verify", json={"user_id": "bob", "facescan": scan_to_b64(_nonced_scan())}
    )
    assert r.status_code == 200, r.json()
    assert r.json()["match"] is False
    assert r.json()["score"] == pytest.approx(0.0, abs=1e-6)


def test_f03_enroll_rejects_multi_face_in_any_embedded_frame(
    fake_pipeline, monkeypatch
):
    def dets(scan):
        frames = _alice_bob_detections()

        frames[1] = [_face("alice", 320, 160, 0.95), _face("bob", 560, 120, 0.5)]
        return frames

    monkeypatch.setattr(fake_pipeline, "_scan_detections", dets)
    scan = _nonced_scan()
    r = client.post(
    ...
    assert challenge.inspect_from_scan(scan).ok is True
...
def test_f40_user_id_is_case_insensitive_across_transports(alice_only):
    r = client.post(
        "/v1/enroll", json={"user_id": "S-01", "facescan": scan_to_b64(_nonced_scan())}
    )
    assert r.status_code == 200, r.json()
    assert store.has_user("s-01") and not store.has_user("S-01")
    v = client.post(
        "/v1/verify",
        content=scan_to_msgpack(_nonced_scan()),
        headers={"Content-Type": "application/msgpack", "X-User-Id": "  s-01 "},
    )
...
@pytest.mark.parametrize("route", ["/v1/enroll", "/v1/verify"])
def test_f22_missing_nonce_takes_the_old_path(no_face_detections, route):
    store.enroll("alice", _VEC["alice"])
    r = client.post(
        route, json={"user_id": "alice", "facescan": scan_to_b64(build_scan())}
    )
    assert no_face_detections, "missing nonce was refused before inference"
    assert r.json()["error"]["code"] == "NO_FACE"


@pytest.mark.parametrize("what", ["params", "action"])
@pytest.mark.parametrize("route", ["/v1/enroll", "/v1/verify"])
def test_f22_mismatch_takes_the_old_path_and_is_not_spent_on_no_face(
    no_face_detections, what, route
):
    store.enroll("alice", _VEC["alice"])
    scan = _mismatched_scan(what)
    r = client.post(route, json={"user_id": "alice", "facescan": scan_to_b64(scan)})
    assert no_face_detections, "mismatch was refused before inference"
    assert r.json()["error"]["code"] == "NO_FACE"
    fresh = dict(scan, challenge=dict(scan["challenge"], **_issued_shape(scan)))
    assert challenge.inspect_from_scan(fresh).ok is True
...
def test_f22_enforcement_off_keeps_the_old_path(monkeypatch, fake_pipeline):
    ...
    r = client.post(
        "/v1/enroll", json={"user_id": "alice", "facescan": scan_to_b64(build_scan())}
    )
    assert r.status_code == 200, r.json()


@requires_models
def test_f22_live_nonce_with_no_face_is_still_no_face_and_unspent(low_det_thresh):
    scan = build_scan(challenge=challenge_map())
    r = client.post(
        "/v1/enroll", json={"user_id": "alice", "facescan": scan_to_b64(scan)}
    )
...
async def _post_scan(ac, route="/v1/enroll"):
    body = {
        "user_id": "alice",
        "facescan": scan_to_b64(build_scan(challenge=challenge_map())),
    }
...
def test_f21_verify_reads_templates_once(fake_pipeline, monkeypatch):
    from app import main

    store.enroll("alice", _VEC["alice"])
    calls = []
    real = store.get_templates

    def spy(user_id):
        calls.append(user_id)
        return real(user_id)

    monkeypatch.setattr(main.store, "get_templates", spy)
    monkeypatch.setattr(
        main.store, "has_user", lambda u: pytest.fail("has_user called")
    )
    monkeypatch.setattr(
        fake_pipeline,
        "_scan_detections",
    ...
    r = client.post(
        "/v1/verify", json={"user_id": "alice", "facescan": scan_to_b64(_nonced_scan())}
    )
    assert r.status_code == 200, r.json()
    assert calls == ["alice"]


def test_f21_delete_during_verify_does_not_500(fake_pipeline, monkeypatch):
    from app import main

    store.enroll("alice", _VEC["alice"])

    def delete_mid_scan(scan):
        store.delete_user("alice")
        return [[_face("alice", 320, 160, 0.9)] for _ in range(12)]

    monkeypatch.setattr(main, "_scan_detections", delete_mid_scan)
    r = client.post(
        "/v1/verify", json={"user_id": "alice", "facescan": scan_to_b64(_nonced_scan())}
    )
    assert r.status_code == 200, r.json()
...
@requires_models
def test_f02_complete_settle_window_still_enrolls(low_det_thresh):
    scan = _scan_with_params("face_like.jpg", {"settle_ms": 2400, "target": 0.25})
    r = client.post(
        "/v1/enroll", json={"user_id": "alice", "facescan": scan_to_b64(scan)}
    )
    assert r.status_code == 200, r.json()
