...
test ! -e "$release"
mkdir "$release"
tar -xzf /tmp/facetech-origin-v5.tar.gz -C "$release"
...
stage=new/'.env.stage'
stage.write_text(patch(env.read_text(),{'FACETECH_CAPTURE_DIR':'/var/lib/facetech/origin-v5-stage'}));stage.chmod(0o600)
print('Gateway configuration, captures, credentials and retention preserved; engine image retained')
PY
install -d -m 700 -o 10002 -g 10002 /var/lib/facetech/origin-v5-stage
cd "$release"
docker compose --env-file .env -f docker-compose.demo.yml -p facetech-demo build gateway
docker compose --env-file .env.stage -f docker-compose.demo.yml -p facetech-origin-stage up -d --no-deps gateway
