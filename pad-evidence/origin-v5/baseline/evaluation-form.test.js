...
async function setup(value=ready) {
  ...
  vm.runInNewContext(code,{window,Event,crypto,AbortController,document:{getElementById:id=>elements[id]},fetch,setTimeout:clock.setTimeout,clearTimeout:clock.clear});
  await clock.drain();
  return {window,e:elements,clock,calls,state};
}
