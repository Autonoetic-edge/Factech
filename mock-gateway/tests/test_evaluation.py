...
def evaluation(tmp_path, monkeypatch):
    ...
    def engine(request):
        ...
        decision = {
            ...
            "pad": {
                "policy": "pad-sequence-v1",
                "outcome": "spoof",
                "usable": 12,
                "frames": [{"index": 0, "scores": [0.8, 0.1, 0.1]}],
            },
            "head_sequence": {"ok": False, "reason": "action_order_or_rotation"},
        }
...
def test_rejected_capture_is_linked_to_diagnostics_and_frames(evaluation):
    ...
    assert detail.json()["decision"]["pad"]["outcome"] == "spoof"
    assert detail.json()["decision"]["pad"]["frames"][0]["scores"] == [0.8, 0.1, 0.1]
    assert not detail.json()["decision"]["head_sequence"]["ok"]
