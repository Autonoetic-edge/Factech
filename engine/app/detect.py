...
def decode_jpeg(jpeg_bytes: bytes):
    from app.facescan import MAX_FRAME_LONG_EDGE, MAX_FRAME_PIXELS, jpeg_dimensions

    dims = jpeg_dimensions(jpeg_bytes)
    if (
        dims is None
        or min(dims) <= 0
        or max(dims) > MAX_FRAME_LONG_EDGE
        or dims[0] * dims[1] > MAX_FRAME_PIXELS
    ):
        return None
