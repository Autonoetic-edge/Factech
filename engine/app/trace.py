...
# The page's last pre-check reading before auto-start (apps/verify), for the team's
# light/blur limits. Advisory data from the browser: recorded, never trusted or used.
PRECHECK_HEADER = "X-Facetech-Precheck"
PRECHECK_FIELDS = ("luma", "frame_luma", "backlight", "sharp", "hint", "capped")
MAX_PRECHECK_CHARS = 256
...
def precheck_of(raw) -> dict | None:
    """Known pre-check fields as scalars; anything malformed is recorded as unreadable."""
    if raw is None:
        return None
...
class Trace:
    ...
    def pad(self, result) -> None:
        self.fields["pad"] = result

    def sequence(self, result) -> None:
        self.fields["head_sequence"] = result

    def geometry(self, record) -> None:
        self.fields["geometry"] = record

    def ensemble(self, record) -> None:
        self.fields["ensemble"] = record

    def flash(self, record) -> None:
        self.fields["flash"] = record

    def enrolment(self, record) -> None:
        self.fields["enrolment"] = record

    def precheck(self, raw) -> None:
        value = precheck_of(raw)
        if value is not None:
            self.fields["precheck"] = value
