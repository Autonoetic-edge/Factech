...
export async function getChallenge(
  ...
  const headers = operation ? { 'X-Facetech-Operation': operation, 'X-User-Id': userId ?? '' } : {};
  const reply = await request(cfg, '/v1/challenge', CHALLENGE_TIMEOUT_MS, runSignal, { method: 'GET', headers });
