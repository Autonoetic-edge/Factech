...
evidence of the M0-M6 freeze. The ACTIVE manifest, on the owner's approval of
...
| .github/workflows/interop.yml (5.3/5.5, conditional) | Only if this workflow exists and pinning dependency versions (5.3) or widening its path filters (5.5) needs a change here rather than only in contract.yml/engine.yml: that minimal change, same constraints as the contract.yml row above. | pending |
| .github/actions/ruff/action.yml (5.3/5.5, conditional) | Only if this action exists and pinning its ruff version (5.3) or widening what triggers it (5.5) needs a change to the action itself rather than only to the workflow calling it: that minimal change. No change to what it lints or how it fails. | pending |
