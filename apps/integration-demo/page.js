import { createFaceSession, CAPTURE_MS, FRAME_COUNT, LANDING_MS, OVAL_START } from '/sdk/index.js?v=ui-v6';
...
for (const id of ['testSubject', 'userId']) {
  $(id)?.addEventListener?.('input', () => cancelAttempt('tester or target changed'));
  $(id)?.addEventListener?.('change', () => cancelAttempt('tester or target changed'));
}
